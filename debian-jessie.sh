...
DEB_RELEASE=jessie
DEB_ARCH=armhf
DEB_EXTRA_PKGS="openssh-server sudo"
DUSER=solidrun
DPASS=solidrun
...
deb http://repo.gbps.io/BSP:/Cubox-i/Debian_Jessie/ ./
deb-src http://repo.gbps.io/BSP:/Cubox-i/Debian_Jessie/ ./
EOF
curl -k http://repo.gbps.io/BSP:/Cubox-i/Debian_Jessie/Release.key | chroot_run apt-key add -

# pin packages from bsp repo with higher priority
cat > build/etc/apt/preferences.d/bsp-cubox-i << EOF
Package: *
Pin: release o=obs://mx6bs/BSP:Cubox-i/Debian_Jessie
Pin-Priority: 600
EOF
...
# make tarball
cd build
tar --numeric-owner -cpvf ../debian-jessie.tar *
cd ..

# compress
pigz debian-jessie.tar
